...
class Sender {
	...
	public:
		Sender(Network *net) {
			...
			{
				//If message = "45 50 90"
				found=message.find(c); // Finds first occurence of c which is whitespace here
				sub=message.substr(0,found); //Get frame i using substring. E.g, if found=2, sub will be 45
				data[i]=sub; //Set data[i] = sub
				message=message.substr(found+1,len);  	  //Remove last obtained substring. 
			}
		}
		void transmit() 
		{
