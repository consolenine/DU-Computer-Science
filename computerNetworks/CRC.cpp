...
int main() {
	...
	for (int i = 0; i < 10; i++) {
		int random = (int)rand() % 16; // Generate a random number between 0 to 16
		cout << "SentWord           ReceivedWord" << endl;
		//Copy codeword to a new variable sentWord which will be sent to CRCChecker
		string sentWord = codeword;
		cout << codeword << "     ";
		if (random < codeword.length()) { // If random is less than codeword length, flip a random bit. If not, then dont change codeword.
			codeword[random] == '0' ? sentWord.replace(random, 1, "1") : sentWord.replace(random, 1, "0"); //Flip i bit of sentWord where i is random number generated
		}
		//If sentWord is changed(to check for errors, CRCChecker will say error)
		//If sentWord is not changed CRCChecker wont give errors
